...
std_msgs::UInt64 msg_sending_time;
std_msgs::Time msg_receiving_time;
...
void receiveTimeCallback (const std_msgs::Time::ConstPtr& msg) {
  msg_receiving_time = *msg;  
}

void sendTimeCallback (const std_msgs::UInt64::ConstPtr& msg) {
  msg_sending_time = *msg;  
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber joint_mcs_sub = node_handle.subscribe("/jointAnglesFromFile/JointPosition", 1, jointMcsCallback);
  ros::Subscriber pose_mcs_sub = node_handle.subscribe("/poseFromFile/PoseStamped", 1, poseMcsCallback);
  ros::Subscriber receive_sub = node_handle.subscribe("/iiwa/poseFromFile/receiveTime", 1, receiveTimeCallback);
  ros::Subscriber send_sub = node_handle.subscribe("/iiwa/timeSendingCommand", 1, sendTimeCallback);
  ...
  ros::Publisher receive_pub = node_handle.advertise<std_msgs::Time>("published_receiving_time", 10);
  ros::Publisher send_pub = node_handle.advertise<std_msgs::UInt64>("published_sending_time", 10);
  ...
  while (ros::ok())
  {
    ...
    receive_pub.publish(msg_receiving_time);
    send_pub.publish(msg_sending_time);
